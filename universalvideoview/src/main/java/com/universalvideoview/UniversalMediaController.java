...
//    private boolean mFullscreenEnabled = false;
    ...
    private void initControllerView(View v) {
        ...
        lock = (ImageView) findViewById(R.id.center_lock_btn);
        ...
        lock.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (isLocked) {
                    Log.d("zzzzz","lock-----");
                    isLocked = false;
                    lock.setImageResource(R.drawable.ic_lock_circle_normal_o);
                    isCallFromBtnClick = true;
                } else { /** 锁屏 */
                    isLocked = true;
                    hide();
                    lock.setImageResource(R.drawable.ic_locked_circle_normal_o);
                }
    ...
    private long timeClick = 0; /** 锁屏情况 点击屏幕的时间差 */
    public void show(int timeout) {//只负责上下两条bar的显示,不负责中央loading,error,playBtn的显示.
        ...
        if(isLocked){
            ...
            if(lock.getVisibility()==VISIBLE){
                ...
                //Log.d("qqqqq","timeClick!=0");
                if(timeClick!=0){
                    //Log.d("qqqqq","temp: "+temp+" tc: "+timeClick);
                    if(temp - timeClick < 2000){ // 30 - 10
                        Log.d("qqqqq","hit "+(temp - timeClick));
                        lock.setVisibility(GONE); /** 直接隐藏 */
                        return;
                    }
                }
                timeClick = temp;
    ...
    private boolean isCallFromBtnClick = false; /** 是否从 锁屏按钮 出点击过来 */
    public void hide() {//只负责上下两条bar的隐藏,不负责中央loading,error,playBtn的隐藏
        if (mShowing) {
            if(isLocked){
                isCallFromBtnClick = false;
                /** 延时隐藏 */
                mHandler.sendEmptyMessageDelayed(0x123,3000);
            }else{
                /** 直接快速隐藏 */
    ...
    public boolean onTouchEvent(MotionEvent event) {
        /** 触屏隐藏 控件的 操作全部 迁移到 onTouch 里面处理 */
...
//        }
    ...
    private OnTouchListener mTouchListener = new OnTouchListener() {
        public boolean onTouch(View v, MotionEvent event) {
            Log.d("zzzzz","onTouch-----");
            if (VideoViewHelper.gestureDetector.onTouchEvent(event))
                return true;

            // 处理手势结束
            switch (event.getAction() & MotionEvent.ACTION_MASK) {
                case MotionEvent.ACTION_UP:
                    VideoViewHelper.volume = -1;
                    VideoViewHelper.brightness = -1f;
                    if(mPlayer!=null){
                        mPlayer.onTouchUp(v);
                    }
    ...
    private View.OnClickListener mScaleListener = new View.OnClickListener() {
        public void onClick(View v) {
            ...
            if(FullScreenListener!=null) {
                FullScreenListener.setFullscreen(mIsFullScreen);
            }
        }
    };
    ...
    private View.OnClickListener mBackListener = new View.OnClickListener() {
        public void onClick(View v) {
            if (mIsFullScreen) {
                ...
                updateBackButton();
                if(FullScreenListener!=null) {
                    FullScreenListener.setFullscreen(false);
                }
            }
...
//            mCenterPlayButton.setVisibility(VISIBLE);
    ...
    private boolean isFromOutSidePause = false;
    public void pauseFromOutSide(){
        if(mPlayer!=null) {
            if (mPlayer.isPlaying()) {
                Log.d("zzzzz","mPlayer.isPlaying()");
                isFromOutSidePause = true;
                mPlayer.pause();
            }
            updatePausePlay();
        }
    ...
    public void resumeFromOutSide(){
        if(mPlayer!=null && isFromOutSidePause){
            isFromOutSidePause = false;
            mPlayer.start();
            updatePausePlay();
        }
